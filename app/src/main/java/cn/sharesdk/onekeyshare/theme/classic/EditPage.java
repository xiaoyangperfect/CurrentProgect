...
public class EditPage extends EditPageFakeActivity implements OnClickListener, TextWatcher {
	...
	private static final int DIM_COLOR = R.color.button_color;
...
//		rlThumb.setId(1);
	...
	private void genBackground() {
		background = new ColorDrawable(activity.getResources().getColor(DIM_COLOR));
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
			...
			win.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN
					| WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
			rlPage.setBackgroundColor(activity.getResources().getColor(DIM_COLOR));
